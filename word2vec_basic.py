...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  window_size = 1+2*(skip_window)
  sample_index = 0
  window_start_index = 0
  
  while (sample_index<=batch_size-window_size+1):
    mid = int(window_start_index + window_start_index+window_size)/2
    temp = num_skips
    for i in range(1, skip_window+1):
      if temp>0:
        batch[sample_index] = mid
        labels[sample_index] = mid-i
        sample_index += 1
        temp -= 1
      else:
        break
...
if __name__ == '__main__':
  ...
  valid_examples = [46, 64, 74]   #IDs for words {first, american, would}. This is to identify top 20 similar words for each of them 
